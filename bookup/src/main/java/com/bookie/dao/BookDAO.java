...
public class BookDAO extends BaseDAO<Book, String> {
    ...
    public List<Book> filterBooks(String genre, String publisher, int year, String author) {
        ...
        StringBuilder query = new StringBuilder(
            "SELECT b.ISBN, b.title, b.year, b.publisher, b.isFeatured, " +
            "a.authorID, a.name AS authorName, b.genreID, b.genreName " +
            "FROM Books b " +
            "LEFT JOIN Authors a ON b.authorID = a.authorID WHERE 1=1"
        );

        // Add conditions based on filters
        if (genre != null && !genre.isEmpty())
            query.append(" AND b.genreName LIKE ?");
        if (publisher != null && !publisher.isEmpty())
            query.append(" AND b.publisher = ?");
        if (year > 0)
        ...
        try (PreparedStatement stmt = connection.prepareStatement(query.toString())) {
            ...
            while (rs.next()) {
                ...
                if (rs.getInt("authorID") != 0)
                    authorObj = new Author(rs.getInt("authorID"), rs.getString("authorName"));

                // Fetch Genre object
                Genre genreObj = new Genre(rs.getInt("genreID"), rs.getString("genreName"));
